...
class GetWaiCurrencyKline_Weekly extends Command
{
	protected $signature = "get_waikui_kline_data_weekly";
        ...
        foreach ($ar as $vv) {
			...
			foreach ($ar_new as $it) {
				// if (in_array($it['name'], $trade)) {
					$data = array();
					$data = $this->get_history_kline($it['name'], 'weekly', 1);
					if ($data['code'] == '200') {
						$info = $data['data'][0];
						$insert_instance = DB::table('market_hour')->where('currency_id', $it['currency_id'])->where('legal_id', $it['legal_id'])->where('day_time', '=',  strtotime($info['date']))->where('period', '1week')->where('sign', 2)->where('type', 8)->first();
