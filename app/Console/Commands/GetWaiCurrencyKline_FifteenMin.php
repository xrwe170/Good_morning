...
class GetWaiCurrencyKline_FifteenMin extends Command
{
	protected $signature = "get_waikui_kline_data_fifteenmin";
		  ...
		  foreach ($ar as $vv) {
			...
			foreach ($ar_new as $it) {
				// if (in_array($it['name'], $trade)) {
					...
					if ($data['code'] == '200') {
						$info = $data['data'][0];
						$insert_instance = DB::table('market_hour')->where('currency_id', $it['currency_id'])->where('legal_id', $it['legal_id'])->where('day_time', '=', strtotime($info['date']))->where('period', '15min')->where('sign', 2)->where('type', 1)->first();
