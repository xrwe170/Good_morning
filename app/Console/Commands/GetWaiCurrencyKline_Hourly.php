...
class GetWaiCurrencyKline_Hourly extends Command
{
	protected $signature = "get_waikui_kline_data_hourly";
...
// 			}
			...
			$info = $data['data'][0];
			$insert_instance = DB::table('market_hour')->where('currency_id', $it['currency_id'])->where('legal_id', $it['legal_id'])->where('day_time', '=', strtotime($info['date']))->where('period', '60min')->where('sign', 2)->where('type', 2)->first();
