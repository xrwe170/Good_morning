...
class GetWaiCurrencyKline_FiveMin extends Command
{
	protected $signature = "get_waikui_kline_data_fivemin";
...
// 			if (in_array($it['name'], $trade)) {
				...
				$data = $this->get_history_kline($it['name'], '5min', 1);
					file_put_contents("ar_new1.txt",'结果:'. json_encode($data) . PHP_EOL, FILE_APPEND);
				if ($data['code'] != '200') {
					continue 1;
				}
				$info = $data['data'][0];
				$insert_instance = DB::table('market_hour')->where('currency_id', $it['currency_id'])->where('legal_id', $it['legal_id'])->where('day_time', '=',strtotime($info['date']))->where('period', '5min')->where('sign', 2)->where('type', 6)->first();
