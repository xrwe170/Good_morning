<?php

return array(
    '退出登录成功' => '正常にログアウトします',
    '电话不能为空' => '電話が必要',
    '账号已存在' => 'アカウントはすでに存在しています',
    '两次密码不一致' => '2つのパスワードに一貫性がありません',
    '用户未找到' => 'ユーザーが見つかりません',
    '钱包已添加,请勿重复添加' => 'ウォレットは追加されています。繰り返し追加しないでください。',
    '添加成功' => '正常に追加されました',
    '参数错误' => 'パラメータエラー',
    '支付密码错误' => '間違った支払いパスワード',
    '余额不足' => '残高不足です',
    '转账至交易所钱包' => '取引所のウォレットに資金を転送する',
    '转账成功' => '転送の成功',
    '找不到用户' => 'ユーザーが見つかりません',
    '用户钱包未找到' => 'ユーザーウォレットが見つかりません',
    '接收来自交易所的转账' => '取引所からの送金を受け取る',
    '您还没有钱包' => 'あなたは財布を持っていません',
    '请输入正确的值' => '正しい値を入力してください',
    '地址输入有误' => 'アドレスが間違って入力されている',
    '地址输入有误1' => 'アドレスが間違って入力されている1',
    '不能转账给自己' => '自分に送金できません',
    '暂时还没有添加币种' => '通貨はまだ追加されていません',
    '此币种不存在' => 'この通貨は存在しません',
    '已经有此提币地址' => 'この撤退アドレスはすでにあります',
    '添加提币地址成功' => '引き出しアドレスの追加に成功しました',
    '此提币地址不存在' => 'この引き出し先住所は存在しません',
    '您没有权限删除此地址' => 'このアドレスを削除する権限がありません',
    '删除提币地址成功' => '引き出しアドレスを正常に削除します',
    '输入的金额不能为负数' => '入力する金額を負の数にすることはできません',
    '您有正在进行中的杆杠交易,不能进行此操作' => '進行中のレバレッジトランザクションがあり、この操作を実行できません',
    '钱包不存在' => 'ウォレットが存在しません',
    '划转成功' => '転送の成功',
    '币种不存在' => '通貨は存在しません',
    '您还没有添加提币地址' => '出金先住所を追加していません',
    '数量不能少于最小值' => '数量は最小値より少なくすることはできません',
    '提币申请已成功，等待审核' => '撤回申請は成功し、審査を待っています',
    '类型错误' => 'タイプエラー',
    '钱包未找到' => 'ウォレットが見つかりません',
    '数量不能小于等于0' => '数量は0以下にすることはできません',
    '价格不能小于等于0' => '価格は0以下にすることはできません',
    '参数错误哦' => 'パラメータエラー',
    '数量不足' => '数量が不足しています',
    '操作成功' => '実行成功',
    '真实姓名必须填写' => '本名を入力する必要があります',
    '收款信息至少选填一项' => '支払い情報を少なくとも1つ入力してください',
    '保存成功' => '正常に保存',
    '密码错误' => '間違ったパスワード',
    '当前行情小于等于零!' => '現在の市場はゼロ以下です！',
    '参数错误!' => 'パラメータエラー！',
    '兑换数量大于持有资产!' => '交換金額は保有資産を上回っています！',
    '资产兑换成功!' => '資産交換は成功です！',
    '兑换数量大于剩余数量!' => '交換数量が残り数量より多いです！',
    '通证兑换成功!' => 'トークン交換が成功しました！',
    '必填项信息不完整' => '必要な情報が不完全です',
    '两次输入密码不一致' => '2つのパスワードに一貫性がありません',
    '此用户不存在' => 'このユーザーは存在しません',
    '此交易账号已经设置' => 'この取引口座は設定されています',
    '此账号已经存在' => 'このアカウントはすでに存在します',
    '交易账号设置成功' => '取引口座が正常に設定されました',
    '验证码错误' => '検証コードエラー',
    '手机绑定成功' => '携帯電話のバインドに成功',
    '邮箱绑定成功' => 'メールボックスのバインドに成功',
    '手势密码至少连接6个点' => 'ジェスチャパスワードには少なくとも6つのポイントが接続されている必要があります',
    '两次手势密码不相同' => '2つのジェスチャーパスワードが同じではありません',
    '手势密码添加成功' => 'ジェスチャーパスワードが正常に追加されました',
    '取消手势密码成功' => 'ジェスチャーパスワードを正常にキャンセルする',
    '密码只能在6-16位之间' => 'パスワードは6〜16桁のみです',
    '交易密码设置成功' => 'トランザクションパスワードが正常に設定されました',
    '暂时还没有邀请排行榜，快去邀请吧' => '招待リーダーボードはまだありません。行って招待してください',
    '会员未找到' => 'メンバーが見つかりません',
    '你已绑定，不可更改!' => 'あなたは縛られており、変えることはできません！',
    '该地址已被绑定,请重新输入' => 'アドレスがバインドされています。再入力してください',
    '绑定成功!' => 'バインディングは成功しました！',
    '请提交完整的信息' => '完全な情報を提出してください',
    '请输入合法的身份证号码' => '法定ID番号を入力してください',
    '该身份证号已实名认证过!' => 'ID番号は他人の本名で確認済みです！',
    '您已经申请过了' => 'あなたはすでに申請しました',
    '提交成功，等待审核' => '正常に送信され、レビューを待っています',
    '您还没有交易记录' => '取引記録がありません',
    '非法操作,不能撤回非自己发布的信息' => '違法な操作、自分で投稿していない情報を取り下げることはできません',
    '非法操作:(' => '違法な操作:(',
    '取销卖出交易失败' => '販売取引のキャンセルに失敗しました',
    '取消成功' => '成功をキャンセル',
    '非法操作' => '違法な操作',
    '数据未找到' => 'データが見つかりません',
    '请先添加钱包' => '最初に財布を追加してください',
    '价格和数量必须大于0' => '価格と数量は0より大きくなければなりません',
    '您的币不足' => 'コインが足りません',
    '您的冻结资金异常，禁止挂卖' => 'あなたの凍結された資金は異常です、それは売ることを禁じられています',
    '你今天的交易额度已达到上限!' => '今日のあなたの取引制限は上限に達しました！',
    '系统配置错误，请联系系统管理员' => 'システム構成エラー、システム管理者に連絡してください',
    '手机号不能为空' => '電話番号を空にすることはできません',
    '发送成功' => '正常に送信されました',
    '账号错误' => 'アカウントエラー',
    '请填写手机号' => '電話番号を入力してください',
    '参数不全' => '不完全なパラメータ',
    '服务器空间不支持,请确认支持curl或者fsocket，联系您的空间商解决或者更换空间！' => 'サーバースペースはサポートされていません。curlまたはfsocketがサポートされていることを確認し、スペースプロバイダーに連絡して、スペースを解決または交換してください。',
    '帐户已过期' => 'アカウントの有効期限が切れました',
    'IP地址限制' => 'IPアドレスの制限',
    '账号被禁用' => 'アカウントが無効になっています',
    '内容含有敏感词' => 'コンテンツに機密性の高い単語が含まれている',
    'ip参数不正确' => 'ipパラメータが正しくありません',
    '邮箱不能为空' => 'メールボックスに記入する必要があります',
    '操作错误' => '操作エラー',
    '无此币种' => 'そのような通貨はありません',
    '该币不是法币' => '通貨は法定通貨ではありません',
    '用户名不能为空' => 'ユーザー名を空にすることはできません',
    '名称不能为空' => '名前は必須です',
    '资产不能为空' => 'コンテンツを空白にすることはできません',
    '找不到此交易账号的用户' => 'この取引口座のユーザーが見つかりません',
    '此用户还未通过实名认证' => 'このユーザーは実名認証に合格していません',
    '此用户已是此法币商家' => 'このユーザーはすでにこの法定通貨の商人です',
    '新闻不存在' => 'ニュースは存在しません',
    '下单失败:超过最大持仓笔数限制' => '注文に失敗しました：最大ポジション数の制限を超えました',
    '当前未获取到行情' => '市場は現在利用できません',
    '尚未申购或理赔保险' => 'まだ加入していないか、保険に加入していない',
    '申购的保险T+1生效' => '購入した保険T + 1が有効になります',
    '受保资产为零' => 'ゼロ保険資産',
    '受保资产小于等于可下单条件' => '被保険者の資産は、配置できる条件以下です。',
    '您已超过持仓限制，暂停下单。' => 'ポジション制限を超え、注文が一時停止されました。',
    '未知的险种类型' => '不明な種類の保険',
    '超过最大持仓数量限制' => 'ポジション制限の最大数を超えています',
    '交易中的订单大于最大挂单数量' => 'トランザクション内の注文が保留中の注文の最大数よりも多い',
    '请输入账号' => 'ユーザー名を入力してください',
    '请输入密码' => 'パスワードを入力してください',
    '手势密码错误' => 'ジェスチャーパスワードエラー',
    '请填写正确的邀请码' => '正しい招待コードを入力してください',
    '注册成功' => '登録成功',
    '请输入密码或确认密码' => 'パスワードの入力または確認をお願いします',
    '输入两次密码不一致' => '2つのパスワードに一貫性がありません',
    '账号不存在' => 'アカウントが存在しません',
    '验证码不正确' => '確認コードが正しくありません',
    '修改密码成功' => 'パスワードの変更に成功',
    '请输入验证码' => '確認コードを入力してください',
    '验证成功' => '正常に検証されました',
    '父级不存在' => '親が存在しません',
    '请重试' => 'もう一度やり直してください',
    '参数错误:(' => 'パラメータエラー:(',
    '缺少参数或传值错误' => 'パラメータがないか、値が間違っています',
    '指定交易对不存在' => '指定されたトレーディングペアは存在しません',
    '您未开通本交易对的交易功能' => 'このトレーディングペアのトレーディング機能を有効にしていません',
    '手数必须是大于0的整数' => 'ロットサイズは0より大きい整数である必要があります',
    '选择倍数不在系统范围' => '乗数の選択はシステム全体ではありません',
    '您有正在平仓中的交易,暂不能进行买卖' => 'ポジションをクローズする過程にある取引があり、一時的に売買することはできません',
    '交易类型错误' => '間違ったトランザクションタイプ',
    '该功能暂未开放' => 'この機能はまだ開いていません',
    '限价交易价格必须大于0' => '指値取引価格は0より大きくなければなりません',
    '当前没有获取到行情价格,请稍后重试' => '現在、市場価格は取得されていません。しばらくしてからもう一度お試しください',
    '限价交易卖出不能低于当前价' => '指値取引は現在の価格を下回って販売することはできません',
    '限价交易买入价格不能高于当前价' => '指値取引の購入価格は現在の価格より高くすることはできません',
    '钱包未找到,请先添加钱包' => 'ウォレットが見つかりません。まずウォレットを追加してください',
    '提交失败' => '送信に失敗しました',
    '提交成功' => '正常に送信されました',
    '此功能系统未开放' => 'この機能システムは開いていません',
    '止盈止损价格不能为0' => 'ストッププロフィットとストップロスの価格を0にすることはできません',
    '找不到该笔交易' => '找不到该笔交易',
    '买入(做多)止盈价不能低于开仓价和当前价' => '買い（ロング）テイク利益価格は始値と現在の価格より低くすることはできません',
    '买入(做多)止亏价不能高于开仓价和当前价' => '買い（長く行く）ストップロス価格は始値と現在の価格より高くすることはできません',
    '卖出(做空)止盈价不能高于开仓价和当前价' => '売り（ショート）テイク利益価格は始値と現在の価格より高くすることはできません',
    '卖出(做空)止亏价不能低于开仓价和当前价' => '売り（ショート）ストップロス価格は始値と現在の価格より低くすることはできません',
    '设置成功' => '正常に設定されました',
    '设置失败' => '設定に失敗しました',
    '无权操作' => '操作する権利がありません',
    '交易状态异常,请勿重复提交' => '取引状況が異常ですので、繰り返し提出しないでください',
    '平仓失败,请重试' => '位置を閉じることができませんでした。もう一度やり直してください',
    '买入方向传参错误' => '購入方向に渡される誤ったパラメーター',
    '提交成功,请等待系统处理' => '送信は成功しました。システムが処理するまでお待ちください',
    '未找到需要平仓的交易' => '閉じる必要のあるトランザクションが見つかりませんでした',
    '交易不存在或已撤单,请刷新后重试' => 'トランザクションが存在しないか、注文がキャンセルされました。更新して再試行してください',
    '撤单失败:用户钱包不存在' => '注文のキャンセルに失敗しました：ユーザーウォレットが存在しません',
    '撤单失败:变更状态失败' => '注文のキャンセルに失敗しました：ステータスの変更に失敗しました',
    '撤单成功' => 'キャンセルに成功',
    '订单取消成功' => '注文は正常にキャンセルされました',
    '请选择需求类型' => '要件の種類を選択してください',
    '请选择交易方式' => '取引方法を選択してください',
    '请填写单价' => '単価をご記入ください',
    '请填写数量' => '数量をご記入ください',
    '请填写最小交易数量' => '最小取引数量を入力してください',
    '请选择币种' => '通貨を選択してください',
    '最小交易数量不能大于总数量' => '最小取引数量は合計数量を超えることはできません',
    '请填写最大交易量' => '最大取引量を入力してください',
    '请填写正确的最大交易量' => '正しい最大取引量を入力してください',
    '对不起，您不是该法币的商家' => '申し訳ありませんが、あなたは法定通貨の商人ではありません',
    '对不起，您的商家账户不足' => '申し訳ありませんが、マーチャントアカウントが不十分です',
    '发布成功' => '成功',
    '无此商家' => 'そのような店はありません',
    '参数错误2' => 'パラメータエラー2',
    '无此记录' => 'そのような記録はありません',
    '购买参数错误' => '購入パラメータエラー',
    '请填写购买额' => '購入金額をご記入ください',
    '购买额请填写数字' => '購入数量をご記入ください',
    '未完成单子超过3单，请完成后再操作!' => '未完成の注文が3つ以上ありますので、操作する前に完了してください。',
    '此条交易已完成' => 'この取引は完了しました',
    '非法提交，数量必须大于0' => '不正な提出、数量は0より大きくなければなりません',
    '您低于最低限额' => 'あなたは最低額を下回っています',
    '您高于最高限额' => 'あなたは上限を超えています',
    '您高于最大限制数量' => 'あなたは上限を超えています',
    '未找到该商家' => 'ビジネスが見つかりませんでした',
    '不能操作自己的' => '自分で操作することはできません',
    '您无此钱包账号' => 'あなたはこのウォレットアカウントを持っていません',
    '您的钱包已被锁定，请联系管理员' => 'ウォレットがロックされています。管理者に連絡してください',
    '您的法币余额不足' => '法定通貨の残高が不足しています',
    '您的法币冻结资金异常,请查看您是否有正在进行的挂单' => '法定紙幣の凍結資金が異常です。保留中の注文があるかどうかを確認してください',
    '您出售数量大于商家剩余数量!' => '販売する数量が販売者の残りの数量よりも多い!',
    '您购买数量大于商家剩余数量!' => 'あなたの購入数量は商人の残りの数量よりも多いです!',
    '您不是此币商家' => 'あなたはこの通貨の商人ではありません',
    '该订单已操作过，请勿重复操作' => '注文は実行中です。操作を繰り返さないでください',
    '对不起，您无权操作' => '申し訳ありませんが、あなたは操作する権限がありません',
    '操作成功，请联系卖家确认' => '操作は成功しました、確認するために売り手に連絡してください',
    '该订单已操作，请勿取消' => '注文は処理されました。キャンセルしないでください',
    '操作成功，订单已取消' => '操作は成功しました、注文はキャンセルされました',
    '对不起您不是商家' => '申し訳ありませんが、あなたはビジネスではありません',
    '对不起，您不是该商家' => '申し訳ありませんが、あなたはビジネスではありません',
    '该订单还未付款或已经操作过' => '注文が支払われていないか、処理されています',
    '订单异常' => '注文の例外',
    '您不能确认该订单' => '注文を確認できません',
    '该用户没有此币种钱包' => 'ユーザーはこの通貨ウォレットを持っていません',
    '确认成功' => '成功を確認する',
    '商家不存在' => 'ストアは存在しません',
    '已撤回发布' => 'リリースは取り下げられました',
    '该发布信息下有交易产生无法删除' => '投稿された情報の下で生成された削除できないトランザクションがあります',
    '撤回成功' => '失効は成功しました',
    '该发布信息下有交易未完成，不能标记为异常' => '投稿された情報の下に未完了の取引があり、異常としてマークすることはできません',
    '该发布信息无异常' => '公開されている情報は正常です',
    '对不起，您的商家账户冻结资金不足' => '申し訳ありませんが、マーチャントアカウントの資金が不足しています',
    '处理成功' => '正常に処理されました',
    '该币种不支持购买保险' => 'この通貨は保険の購入をサポートしていません',
    '错误的金额！' => '金額が間違っています！',
    '不存在的险种！' => '存在しない保険タイプ！',
    '不存在的币种！' => '存在しない通貨！',
    '已经购买了该币种的险种！' => 'この通貨の保険をすでに購入しています！',
    '可用余额不足，无法购买！' => '利用可能な残高が購入するには不十分です！',
    '购买成功！' => '購入の成功！',
    '未找到该保险' => '保険が見つかりませんでした',
    '该保险正在处理中' => '保険は処理中です',
    '申请索赔成功！' => '成功した主張！',
    '超出今日索赔次数!' => '今日のクレーム数を超えています!',
    '存在未平仓订单' => 'オープンオーダーがあります',
    '受保资产不符合可申请索赔条件1' => '被保険資産が請求可能な条件を満たしていない1',
    '受保资产不符合可申请索赔条件2' => '被保険資産が請求可能な条件を満たしていない2',
    '未知受保金额去向状态' => '不明な保険金額の所在状況',
    '处理成功！' => '正常に処理されました！',
    '该保险已失效' => '保険の有効期限が切れています',
    '该保险正在索赔处理中' => '保険は請求処理中です',
    '解约失败，存在未平仓订单' => 'キャンセルに失敗しました。未処理の注文があります',
    '保险解约，赔付金额' => '保険の解約、補償額',
    '解约成功！' => '正常終了！',
    '内容不能为空' => 'コンテンツを空白にすることはできません',
    '暂无数据' => 'データなし',
    '文件大小超出' => 'ファイルサイズを超えました',
    '文件类型非法' => '不正なファイルタイプ',
    '文件类型不对' => 'ファイルの種類が正しくありません',
    '该文件已存在' => 'ファイルはすでに存在します',
    '币未找到' => 'コインが見つかりません',
    '转账账户不存在!' => '転送アカウントが存在しません!',
    '不能给自己转账!' => '自分に送金できません!',
    '转账数量大于剩余数量!' => '振込金額が残額より多い!',
    '余额不足!' => '残高不足です!',
    '通证转账成功!' => 'パス転送が成功しました!',
    '请输入支付密码' => '支払いパスワードを入力してください',
    '无需升级' => 'アップグレードする必要はありません',
    '暂无钱包' => '財布なし',
    '升级成功' => '更新に成功しました',
    '该地址别人已经绑定过了' => 'このアドレスはすでに他の人によってバインドされています',
    '请把参数填写完整' => 'パラメータを完全に入力してください',
    '用户已存在' => 'このユーザーは既に存在します',
    '推荐用户不存在' => '推奨ユーザーが存在しません',
    '转入账户不能为空' => '転送アカウントは空にすることはできません',
    '密码不能为空' => 'パスワードを空白にすることはできません',
    '转入数量不能为空' => '転送された数量を空にすることはできません',
    '用户密码错误' => 'ユーザーパスワードが間違っています',
    '转入成功' => '正常に転送されました',
    '密码修改成功' => 'パスワードのリセットが完了しました',
    '请输入正确的交易数量或价格' => '正しい取引数量または価格を入力してください',
    '用户钱包不存在' => 'ユーザーウォレットが存在しません',
    '用户不存在' => 'ユーザーは存在しません',
    '未找到该发布用户' => '投稿ユーザーが見つかりませんでした',
    '检测有未完成交易，请完成后再来！' => '未完了の取引がないか確認してください。完了後に戻ってきてください。',
    '您的余额不足' => 'あなたのバランスが不十分です',
    '用户信息不正确' => 'ユーザー情報が正しくありません',
    '对不起，这不是您的发布信息' => '申し訳ありませんが、これはあなたの投稿ではありません',
    '对不起，您无权撤销此记录' => '申し訳ありませんが、あなたにはこの記録を取り消す権利がありません',
    '对不起，您的账户冻结资金不足' => '申し訳ありませんが、アカウントに凍結された資金が不足しています',
    '取消次数已超出' => 'キャンセル数を超えました',
    '次,明天再发布' => '数量、明日リリース',
    ',错误位于第' => ',エラーは',
    '行' => '行',
    '您今天的取消次数已超出' => '今日のキャンセル数が超過しました',
    '次' => '量',
    '在 ' => '存在 ',
    '购买法币成功，增加法币余额' => '法定通貨の購入に成功し、法定通貨の残高を増やします',
    '转账数量不能小于' => '振込金額は以下になりません',
    '转账数量不能大于' => '送金金額を超えることはできません',
    '解约失败:' => 'キャンセルに失敗しました:',
    '操作成功，请联系商家确认订单' => '実行は成功しました。注文を確認するために販売者に連絡してください',
    '您还没有通过实名认证' => '実名認証に合格していません',
    '您还没有设置收款信息' => '支払い情報を設定していません',
    '撤单失败:' => 'キャンセルに失敗しました:',
    '扣除保证金失败:' => '証拠金の控除に失敗しました:',
    '扣除手续费失败:' => '手数料の控除に失敗しました:',
    '手数不能低于' => 'ハンドの数は以下にすることはできません',
    '手数不能高于' => 'ハンドの数はこれより多くすることはできません',
    '余额不足,不能小于' => '不十分なバランス、以下にすることはできません',
    '(手续费:' => '(手数料:',
    '此法币' => 'この法定通貨',
    '商家名称已存在' => ',商家名称已存在',
    '请实名认证' => '実名で認証してください',
    '项目不存在' => 'アイテムが存在しません',
    '最少存入数量为：' => '最低入金額は：',
    '当前等级禁止购买30S产品，请联系客服升级' => '現在のレベルでは30S製品の購入は禁止されています。アップグレードするには、カスタマーサービスにお問い合わせください',
    '请登录' => 'サインインしてください',
    '毁约成功' => '契約を無事に破った',
    '旧密码错误' => '古いパスワードエラー',
    '您的实名认证未通过' => '実名認証に失敗しました',
    '下单数量必须是整数' => '注文数量は整数である必要があります',
    '下单数量必须是' => '注文数量は:micro_min の整数倍',
    '最大下单数量不能超过' => '注文の最大数は超えることはできません: :micro_max',
    '最小下单数量不能小于' => '最小注文数量は、: :micro_min',
    '申请成功' => '成功したアプリケーション',
    '认证成功，请等待审核' => '正常に送信されました。審査をお待ちください',
    '操作失败:您的信用分不足' => '操作に失敗しました: クレジット スコアが不十分です :source',
    '操作失败:您有未平仓的交易,操作禁止' => '操作に失敗しました: 開いているトランザクションがあります。操作は禁止されています',
    '支付通道维护,请更换支付方式' => '支払いチャネルのメンテナンス、支払い方法を変更してください',
    '您好，您的账户已被锁定，详情请咨询客服。' => "こんにちは、あなたのアカウントはロックされています。詳細については、カスタマー サービスにお問い合わせください。",
    '您好，您的资金已被锁定，详情请咨询客服。' => "こんにちは、あなたの資金はロックされています。詳細については、カスタマー サービスにお問い合わせください。",
);
