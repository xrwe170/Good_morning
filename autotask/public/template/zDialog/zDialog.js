 ...
 * 样式修正：2015-4-15 李银红
 **/
 //15.5.25,修改默认按钮样式buttonStyle分别为确定popbtnOK，取消popbtnQX，使用的是style.css里的按钮样式
var IMAGESPATH = '/template/zDialog/images/'; //图片路径配置
/*************************一些公用方法和属性****************************/
...
Dialog.prototype.create = function () {
    ...
    var DialogDivHeight = this.Height + 40 + 13 + (this.ShowButtonRow ? 40 : 0) + (this.ShowMessageRow ? 50 : 0);
    ...
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      <td width="13" height="40" style="background-image: url(' + IMAGESPATH + 'dialog_lt.png);"><div style="width: 13px;"></div></td>\
      <td height="40" style="background-image:url(' + IMAGESPATH + 'dialog_ct.png);"><div style="padding: 9px 0 0 4px; float: left; font-weight: bold; color:#fff;"><img align="absmiddle" src="' + IMAGESPATH + 'icon_dialog.png"/><span style="font-size:14px" id="_Title_' + this.ID + '">' + this.Title + '</span></div>\
      ...
      <td width="13" height="40" style="background-image: url(' + IMAGESPATH + 'dialog_rt.png);"><div style="width: 13px;"><a id="_forTab_' + this.ID + '" href="#;"></a></div></td>\
    </tr>\
    <tr valign="top">\
      <td width="13" style="background-image: url(' + IMAGESPATH + 'dialog_mlm.png);"></td>\
      <td><table width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#fff;">\
          ...
          <tr>\
            <td valign="top"><div id="_Container_' + this.ID + '" style="position: relative; width: ' + this.Width + 'px; height: ' + this.Height + 'px;">\
        ...
        return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: right; background-color:#f6f6f6;">\
                <input type="button" class="btn btn-default" value="取 消" style="margin:0 12px;" onclick="Dialog.getInstance(\'' + this.ID + '\').close();" id="_ButtonCancel_' + this.ID + '"/>\
                <input type="button" class="btn btn-info" style="margin:0 12px;" value="确 定" id="_ButtonOK_' + this.ID + '"/>\
              </div></td>\
          </tr>\
        </table></td>\
      <td width="13" style="background-image: url(' + IMAGESPATH + 'dialog_mrm.png);"></td>\
    </tr>\
    <tr>\
      <td width="13" height="13" style="background-image: url(' + IMAGESPATH + 'dialog_lb.png);"></td>\
      <td style="background-image: url(' + IMAGESPATH + 'dialog_cb.png);"></td>\
      <td width="13" height="13" style="background-image: url(' + IMAGESPATH + 'dialog_rb.png);"><a onfocus=\'$id("_forTab_' + this.ID + '").focus();\' href="#;"></a></td>\
    </tr>\
  </table>\
</div>\
'
...
Dialog.prototype.close = function () {
    ...
    if (topWin.Dialog._Array.length > 0) {
        ...
        Dialog.bgDiv.style.zIndex = "9999";
...
Dialog.alert = function (msg, func, w, h) {
    ...
    diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_alert.gif" width="34" height="34" align="absmiddle"></td>\
			<td align="left" id="Message_' + this.ID + '" style="font-size:12px">' + msg + '</td></tr>\
	</table>';
    diag.show();
...
Dialog.confirm = function (msg, funcOK, funcCal, w, h) {
    ...
    diag.InnerHtml = '<table height="100%" border="0" align="center" cellpadding="10" cellspacing="0">\
		<tr><td align="right"><img id="Icon_' + this.ID + '" src="' + IMAGESPATH + 'icon_query.gif" width="34" height="34" align="absmiddle"></td>\
			<td align="left" id="Message_' + this.ID + '" style="font-size:14px">' + msg + '</td></tr>\
	</table>';
    diag.show();
    diag.okButton.parentNode.style.textAlign = "center";
    diag.okButton.focus();
...
//1、全局居中显示
  /*用法：   diytipshow('load','提示内容','y/n',2)
        或：diytipshow('ok','提示内容')，这种形式，将默认为2秒后自动消失
  */
var timelinshi=2;
var tipcontent="";//文本类型，要提示的内容
var hidetimeset=timelinshi;//纯数字，自动消失时间，默认2秒，
var ifAutoHideset="y";//y和n，默认y自动消失，n需要点击才消失
var typeset="load";//图标样式，默认load加载中样式，ok成功，err错误，tip一般提示，
diytipcontent="";
function diytipshow(type,tipContent,ifAutoHide,hideTime,funacc){
	tipcontent=tipContent;
	ifAutoHideset=ifAutoHide;
	hidetimeset=hideTime;
	typeset=type;
	switch(typeset)
	{
	case "load":
	...
	var diytipcontent='<div class="pop_div_markbg"></div><div class="iehack diytipconbox '+icoclass+'"><div class="closebtn"></div><div class="diytipcon">'+tipcontent+'</div></div>';
	$("body").append(diytipcontent);
	diytipconbox_w=$(".diytipconbox").width()/2;
	diytipconbox_h=$(".diytipconbox").height()/2;
	$(".diytipconbox").css({"top":"-100px","margin-top":"-"+(diytipconbox_h+5)+"px","margin-left":"-"+(diytipconbox_w+22)+"px"});
	$(".diytipconbox").animate({"top":"50%"},300);
	if(ifAutoHideset==null){
		ifAutoHideset="y";
	}
	if(hidetimeset==null){
	  hidetimeset=timelinshi;
	}
	if(ifAutoHideset=="y"){
		$(".diytipconbox .closebtn").hide();
		setTimeout("CloseDiytip()",hidetimeset*1000);
	}
	if(ifAutoHideset=="n"){
		$(".pop_div_markbg").click(function(e) {
            CloseDiytip();
        });
		$(".diytipconbox").click(function(e) {
            CloseDiytip();
        });
	}
	   $(".diytipconbox .closebtn").click(function(e) {
            CloseDiytip();
        });
	if (funacc) {
            funacc();
...
function CloseDiytip(){
	$(".diytipconbox").animate({"top":"-100px"},300);
	$(".pop_div_markbg").hide(0);
	$(".diytipconbox").remove();
	$(".pop_div_markbg").remove();
}
