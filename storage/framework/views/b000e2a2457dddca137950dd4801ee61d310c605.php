...
<style>
    ...
    <div style="margin-top: 10px;width: 100%;">
        ...
        <button class="layui-btn layui-btn-normal" style="margin-left: 10px" onclick="javascrtpt:window.location.href='<?php echo e(url('/admin/cashb/csv')); ?>'">导出提币记录</button>
    </div>
...
<?php $__env->startSection('scripts'); ?>
    <script>
        ...
        layui.use(['table','form'], function(){
            ...
            table.render({
                elem: '#demo'
                ,url: "<?php echo e(url('admin/cashb_list')); ?>" //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('真的删除行么', function(index){
                        $.ajax({
                            url:'<?php echo e(url('admin/cashb_show')); ?>',
                            type:'post',
                            dataType:'json',
                            data:{id:data.id},
                            success:function (res) {
                    ...
                    layer_show('确认提币','<?php echo e(url('admin/cashb_show')); ?>?id='+data.id,800,600);
                } else if(obj.event === 'back'){
                    layer_show('退回申请','<?php echo e(url('admin/adjust_account')); ?>?id='+data.id,800,600);
                }
            });
