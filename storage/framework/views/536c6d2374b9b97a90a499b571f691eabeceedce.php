            ...
            <script>
                window.onload = function() {
                    ...
                    layui.use(['element', 'form', 'layer', 'table'], function () {
                        ...
                        tbRend("<?php echo e(url('/admin/user/walletList')); ?>?user_id=" + user_id);
                        ...
                        form.on('switch(sexDemo)', function(obj){
                            ...
                            $.ajax({
                                url:'<?php echo e(url('admin/user/wallet_lock')); ?>',
                                type:'post',
                                dataType:'json',
                                data:{id:id},
                        ...
                        table.on('tool(userlist)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                            ...
                            if (layEvent === 'delete') { //删除
                                layer.confirm('真的要删除吗？', function (index) {
                                    //向服务端发送删除指令
                                    $.ajax({
                                        url: '<?php echo e(url('admin/user/delw')); ?>',
                                        type: 'post',
                                        dataType: 'json',
                                        data: {id: data.id},
                                        success: function (res) {
                                ...
                                var index = layer.open({
                                    title: '调节账户'
                                    , type: 2
                                    , content: '<?php echo e(url('/admin/user/conf')); ?>?id=' + data.id
                                    , maxmin: true
                                });
                                layer.full(index);
                                ...
                                var index = layer.open({
                                    title: '管理提币地址'
                                    , type: 2
                                    , content: '<?php echo e(url('/admin/user/address')); ?>?id=' + data.id
                                    , maxmin: true
                                });
                                layer.full(index);
                            } else if (layEvent === 'balance') { //余额归拢
                                $.ajax({
                                    url: '<?php echo e(url('admin/user/balance')); ?>',
                                    type: 'post',
                                    dataType: 'json',
                                    data: {id: data.id},
                                    success: function (res) {
                                ...
                                $.ajax({
                                    url: '<?php echo e(url('admin/send/btc')); ?>',
                                    type: 'post',
                                    dataType: 'json',
                                    data: {id: data.id},
                                    success: function (res) {
