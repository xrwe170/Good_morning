...
<?php $__env->startSection('scripts'); ?>
    <script>
        ...
        layui.use(['table','form'], function(){
            ...
            table.render({
                elem: '#demo'
                ,url: '<?php echo e(url('admin/legal_deal/list')); ?>' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
             ...
             table.on('tool(test)', function(obj){
                 ...
                 if(obj.event === 'cancel')
                 {
                     layer.confirm('确定要取消吗', function(index){
                         $.ajax({
                             url:'<?php echo e(url('/admin/admin_legal_pay_cancel')); ?>',
                             type:'post',
                             dataType:'json',
                             data:{id:data.id},
...
//                                     obj.del();
                 ...
                 else if(obj.event === 'confirm')
                 {
                     if(data.type=="buy")
                     {
                         ...
                         {
                             $.ajax({
                                 url:'<?php echo e(url('/admin/legal_deal_admin_user_sure')); ?>',
                                 type:'post',
                                 dataType:'json',
                                 data:{id:data.id},
                                 success:function (res) {
...
//                                     obj.del();
                     ...
                     else if(data.type=="sell")
                     {
                         ...
                         {
                             $.ajax({
                                 url:'<?php echo e(url('/admin/legal_deal_admin_sure')); ?>',
                                 type:'post',
                                 dataType:'json',
                                 data:{id:data.id},
                                 success:function (res) {
