...
<?php $__env->startSection('page-content'); ?>
    <div style="margin-top: 10px;width: 100%;margin-left: 10px;">
        <button class="layui-btn layui-btn-normal layui-btn-radius" onclick="layer_show('添加商家','<?php echo e(url('admin/seller_add')); ?>')">添加商家</button>
...
<?php $__env->startSection('scripts'); ?>
    ...
    <script>
        ...
        layui.use(['table','form'], function(){
            ...
            table.render({
                elem: '#demo'
                ,url: '<?php echo e(url('admin/seller_list')); ?>' //数据接口
                ,page: true //开启分页
                ,id:'mobileSearch'
                ,cols: [[ //表头
            ...
            form.on('switch(status)', function(obj){
                var id = this.value;
                $.ajax({
                    url:'<?php echo e(url('admin/seller/status')); ?>',
                    type:'post',
                    dataType:'json',
                    data:{id:id},
                    success:function (res) {
            ...
            table.on('tool(test)', function(obj){
                ...
                if(obj.event === 'del'){
                    layer.confirm('真的删除行么', function(index){
                        $.ajax({
                            url:'<?php echo e(url('admin/seller_del')); ?>',
                            type:'post',
                            dataType:'json',
                            data:{id:data.id},
                            success:function (res) {
                    ...
                    layer_show('编辑商家','<?php echo e(url('admin/seller_add')); ?>?id='+data.id);
                } else if(obj.event === 'detail'){
                    layer_show('调节账户','<?php echo e(url('admin/adjust_account')); ?>?id='+data.id,800,600);
                }
            });
