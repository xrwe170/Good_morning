    ...
    <div style="margin-top: 10px;width: 100%;margin-left: 10px;">
        <form class="layui-form layui-form-pane layui-inline" action="">
            ...
            <div class="layui-inline" style="margin-left: 10px;">
                ...
                <button class="layui-btn layui-btn-normal" onclick="javascrtpt:window.location.href='<?php echo e(url('/admin/Leverdeals/csv')); ?>'">导出记录</button>
            </div>
        </form>
    </div>
        ...
        window.onload = function() {
            ...
            layui.use(['element', 'form', 'layer', 'table'], function () {
                ...
                form.on('submit(mobile_search)',function(obj){
                    ...
                    tbRend("<?php echo e(url('/admin/Leverdeals/list')); ?>?id="+id+"&phone="+phone+"&type="+type+"&status="+status);
                    return false;
                });
                function tbRend(url) {
                    table.render({
                ...
                tbRend("<?php echo e(url('/admin/Leverdeals/list')); ?>");
                ...
                table.on('tool(userlist)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                    ...
                    if (layEvent === 'delete') { //删除
                        layer.confirm('真的要删除吗？', function (index) {
                            //向服务端发送删除指令
                            $.ajax({
                                url: "<?php echo e(url('admin/user/del')); ?>",
                                type: 'post',
                                dataType: 'json',
                                data: {id: data.id},
                                success: function (res) {
                        ...
                        layer_show('编辑会员','<?php echo e(url('admin/user/edit')); ?>?id='+data.id);
                    }else if (layEvent === 'users_wallet') {
                        var index = layer.open({
                            title: '账户管理'
                            , type: 2
                            , content: '<?php echo e(url('/admin/user/users_wallet')); ?>?id=' + data.id
                            , maxmin: true
                        });
                        layer.full(index);
                    }
