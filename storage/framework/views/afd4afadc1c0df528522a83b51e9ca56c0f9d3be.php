...
<script>        
    layui.use(['element', 'form', 'layer', 'table'], function () {
        ...
        $('#add_user_info').click(function(){
            layer_show('添加会员', '/admin/user/add');
            return false
        });
        ...
        form.on('switch(bind_box)', function(obj){
            ...
            $.ajax({
                url:'<?php echo e(url('admin/user/set_bind_box')); ?>',
                type:'post',
                dataType:'json',
                data:{id:id},
        ...
        form.on('switch(is_trader)', function(obj){
            ...
            $.ajax({
                url:'<?php echo e(url('admin/user/trader')); ?>',
                type:'post',
                dataType:'json',
                data:{user_id:id},
                success:function (res) {
        ...
        table.on('tool(userlist)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'delete') { //删除
                ...
                $.ajax({
                    url:'<?php echo e(url('admin/ybb/account')); ?>',
                    type: 'get',
                    dataType: 'json',
                    data: {
                        user_id:data.id
